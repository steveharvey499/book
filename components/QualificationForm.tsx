...
const QualificationForm = () => {
  ...
  // Check if user has already signed up - redirect if so
  useEffect(() => {
    const hasSignedUp = localStorage.getItem('brand-attention-strategy-signed-up') === 'true';
    if (hasSignedUp) {
  ...
  const onFinalSubmit = async (data: QualificationFormData) => {
    ...
    try {
      ...
      // Success - save to localStorage and redirect to Foreword
      ...
      // Small delay to show success message, then redirect to start page
      setTimeout(() => {
        router.push('/book/start');
      }, 1500);
    } catch (error) {
      ...
      <div className="container mx-auto max-w-2xl">
        ...
        <div className="bg-white rounded-lg shadow-xl p-6 sm:p-8">
          <h2 className="text-section text-adenine mb-6 text-center">
            {currentStep === 1 ? "Start Reading Now" : currentStep === 2 ? "Tell Us About Yourself" : "Almost There"}
          </h2>
